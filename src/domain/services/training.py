...
class TrainingService:
    ...
    async def get_training(self, id: UUID) -> DomainTraining:
        training = await self._repo.find_by_id(id)
        if not training:
            raise NotFoundError(f"Training {id} not found")
        return training
    
    async def create_training(self, owner_id: UUID, name: str, description: str) -> DomainTraining:
        ...
        return await self._repo.add_training(training)
    
    async def delete_training(self, id: UUID) -> None:
        training = await self._repo.find_by_id(id)
        if not training:
            raise NotFoundError(f"Training {id} not found")
        await self._repo.delete_training(id)
    ...
    async def update_training(
        ...
        training = await self._repo.find_by_id(training_id)
        if not training:
        ...
        return await self._repo.update_training(training)
    
    async def get_all_owner_trainings(self, owner_id: UUID) -> List[DomainTraining]:
        trainings = await self._repo.find_all_owner_trainings(owner_id)
        if not trainings:
            return []
        return trainings
    ...
    async def create_validate(
        ...
        return await self._repo.add_validate(validate)

    async def get_validates_for_task(self, task_id: UUID) -> List[DomainValidate]:
        validates = await self._repo.find_validate_by_task_id(task_id)
        if not validates:
            return []
        return validates

    async def delete_validate(self, validate_id: UUID) -> None:
        validate = await self._repo.find_validate_by_id(validate_id)
        if not validate:
            raise NotFoundError(f"Validation {validate_id} not found")
        await self._repo.delete_validate(validate_id)

    async def get_validate_by_training_id(self, training_id: UUID) -> List[DomainValidate]:
        if not await self._repo.find_by_id(training_id):
          raise NotFoundError(f"Training {training_id} not found")
        return await self._repo.find_all_validates_by_training_id(training_id)